...
- **Reorder groups** — drag a group's chip to move the whole group along the bar as one block. An insertion line marks where it will land — only ever *between* groups and ungrouped tabs, snapping to a whole group's edge (never between its tabs) and flipping once at the group's midpoint. Drop and it lands before or after that group as a unit — groups reorder *around* each other and never split or merge.
