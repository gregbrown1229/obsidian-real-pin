...
- **Drag to group** — Obsidian already lets you drag a tab along the bar. Membership only changes on an unambiguous move: drop a tab **between** a group's tabs to add it, or drag a member fully **out** of the run to remove it. Dropping at a group's outer edge leaves the tab where it is — to add a tab at an edge, use the right-click menu. (We read the native reorder and update membership — no custom drag layer, no position guessing.)
- **Reorder groups** — drag a group's chip to move the whole group along the bar as one block. While you drag, every group folds to just its chip so you rearrange them without minding how many tabs each holds; they unfold when you drop. Drop anywhere on another group and it lands before or after that group as a unit — groups reorder *around* each other and never split or merge. Aim at the left half to land before, the right half to land after; the left half of a group's chip is how you reach the very start of the bar.
- **Collapse / expand** — click the group chip (collapsed shows just the chip; the member tabs hide). Focusing any tab in a collapsed group expands it again.
- **Self-tidying** — closing a group's last tab removes the group.
